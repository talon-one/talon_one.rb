...
module TalonOne
  class NewIntegrationHubCoupons
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `TalonOne::NewIntegrationHubCoupons` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `TalonOne::NewIntegrationHubCoupons`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [usage_limit, discount_limit, reservation_limit, start_date, expiry_date, limits, application_id, campaign_id, batch_id, number_of_coupons, attributes, valid_characters, coupon_pattern, is_reservation_mandatory, implicitly_reserved, recipient_integration_id, support_request_id, support_request_note].hash
    end
