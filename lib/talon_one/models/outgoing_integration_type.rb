  ...
  class OutgoingIntegrationType
    ...
    def initialize(attributes = {})
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `TalonOne::OutgoingIntegrationType`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@documentation_link.nil? && @documentation_link.to_s.length < 1
        invalid_properties.push('invalid value for "documentation_link", the character length must be great than or equal to 1.')
      end
    ...
    def category=(category)
      if !category.nil? && category.to_s.length > 64
        fail ArgumentError, 'invalid value for "category", the character length must be smaller than or equal to 64.'
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] documentation_link Value to be assigned
    def documentation_link=(documentation_link)
      if !documentation_link.nil? && documentation_link.to_s.length > 255
        fail ArgumentError, 'invalid value for "documentation_link", the character length must be smaller than or equal to 255.'
      end

      if !documentation_link.nil? && documentation_link.to_s.length < 1
        fail ArgumentError, 'invalid value for "documentation_link", the character length must be great than or equal to 1.'
      end
