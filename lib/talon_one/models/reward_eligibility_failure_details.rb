...
module TalonOne
  ...
  class RewardEligibilityFailureDetails
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `TalonOne::RewardEligibilityFailureDetails` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `TalonOne::RewardEligibilityFailureDetails`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      failure_code_validator = EnumAttributeValidator.new('String', ["CONDITION_NOT_MET", "INSUFFICIENT_BALANCE", "CARD_REQUIRED", "PROFILE_REQUIRED"])
    ...
    def failure_code=(failure_code)
      validator = EnumAttributeValidator.new('String', ["CONDITION_NOT_MET", "INSUFFICIENT_BALANCE", "CARD_REQUIRED", "PROFILE_REQUIRED"])
    ...
    def hash
      [failure_code, condition_index].hash
    end
