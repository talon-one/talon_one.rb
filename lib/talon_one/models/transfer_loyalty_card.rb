  ...
  class TransferLoyaltyCard
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `TalonOne::TransferLoyaltyCard` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `TalonOne::TransferLoyaltyCard`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @new_card_identifier.nil?
        invalid_properties.push('invalid value for "new_card_identifier", new_card_identifier cannot be nil.')
      end
      ...
      if @new_card_identifier.to_s.length < 4
        invalid_properties.push('invalid value for "new_card_identifier", the character length must be great than or equal to 4.')
      end
    ...
    def new_card_identifier=(new_card_identifier)
      ...
      if new_card_identifier.to_s.length < 4
        fail ArgumentError, 'invalid value for "new_card_identifier", the character length must be great than or equal to 4.'
      end
