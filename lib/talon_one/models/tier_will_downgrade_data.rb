...
module TalonOne
  class TierWillDowngradeData
    ...
    attr_accessor :points_required_to_remain
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `TalonOne::TierWillDowngradeData` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `TalonOne::TierWillDowngradeData`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'points_required_to_remain')
        self.points_required_to_remain = attributes[:'points_required_to_remain']
      end
    ...
    def list_invalid_properties
      ...
      if @points_required_to_remain.nil?
        invalid_properties.push('invalid value for "points_required_to_remain", points_required_to_remain cannot be nil.')
      end
