  ...
  class AwardGiveawayBlock
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `TalonOne::AwardGiveawayBlock` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `TalonOne::AwardGiveawayBlock`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'giveaway_pool')
        self.giveaway_pool = attributes[:'giveaway_pool']
      end
    ...
    def list_invalid_properties
      ...
      if @giveaway_pool.nil?
        invalid_properties.push('invalid value for "giveaway_pool", giveaway_pool cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @giveaway_pool.nil?
    ...
    def hash
      [id, type, tags, giveaway_pool, profile, on_failure, on_error].hash
    end
