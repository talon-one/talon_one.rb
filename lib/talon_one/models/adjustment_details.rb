  ...
  class AdjustmentDetails
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `TalonOne::AdjustmentDetails` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `TalonOne::AdjustmentDetails`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @selected_price_type.nil?
        invalid_properties.push('invalid value for "selected_price_type", selected_price_type cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @selected_price_type.nil?
    ...
    def hash
      [reference_id, selected_price_type, value].hash
    end
