  ...
  class Reward
    ...
    def initialize(attributes = {})
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `TalonOne::Reward`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @api_name.to_s.length < 1
        invalid_properties.push('invalid value for "api_name", the character length must be great than or equal to 1.')
      end
    ...
    def api_name=(api_name)
      ...
      if api_name.to_s.length < 1
        fail ArgumentError, 'invalid value for "api_name", the character length must be great than or equal to 1.'
      end
    ...
    def hash
      [id, created, account_id, name, api_name, description, application_ids, sandbox, eligibility_conditions, rule, bindings, points_required, modified, status].hash
    end
